...
//session_start();
$file_scanned = 0;
$forbiden_function = ["eval", "system", "create_function", "assert", "shell_exec", "passthru", "exec", "popen", "proc_open", "pcntl_exec"];
if(isset($_GET['act'])){
	if($_GET['act']=="scan"){
...
function scan_file($path){
		...
		foreach($files as $file){
			if (is_dir("$path/$file")) {
				...
				echo "<u>$path/$file</u> is ".($c?'<span class="infected" style="color:red;">infected Shell</span>':'<span class="hehe"  style="color:green;">safe</span>')."<br>\n";
				//if($c==true) die();
